...
def transform_tup_3x3 (p: (f32, f32, f32)) (T: [3](f32,f32,f32)): [3]f32 =
    [
...
def transform_tup_4x3 (p: (f32, f32, f32)) (T: [4](f32,f32,f32)): [4]f32 =
    [
        T[0].0*p.0 + T[0].1*p.1 + T[0].2*p.2,
        T[1].0*p.0 + T[1].1*p.1 + T[1].2*p.2,
        T[2].0*p.0 + T[2].1*p.1 + T[2].2*p.2,
        T[3].0*p.0 + T[3].1*p.1 + T[3].2*p.2
    ]

def transform_point_4x3 (p: [3]f32) (T: [4][4]f32) : (f32, f32, f32) = 
    (
...
def transform_point_4x4  (p: [3]f32) (T: [4][4]f32) : (f32, f32, f32, f32) = 
    (
        T[0][0]*p[0] + T[0][1]*p[1] + T[0][2]*p[2] + T[0][3],
        T[1][0]*p[0] + T[1][1]*p[1] + T[1][2]*p[2] + T[1][3],
        T[2][0]*p[0] + T[2][1]*p[1] + T[2][2]*p[2] + T[2][3],
...
def quat_to_mat (q: [4]f32) : [3][3]f32= 
    let norm = f32.sqrt (q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3])
    let qr = q[0] / norm
    let qi = q[1] / norm
    let qj = q[2] / norm
    let qk = q[3] / norm
